	...
	struct uvc_buffer *buf)
{
	...
	for (i = 0; i < urb->number_of_packets; ++i) {
		...
		/* 20130223 Leon.lin
		  * USB Host will set the recieve data length as  0x3FFE if it receive 
		  * the actual data length > the max packet size
               * sometimes, if the (mem + urb->iso_frame_desc.actual_length) > the mem that alloc in 
               * uvc_init_video_isoc, it will cause kernel fault in memcpy() in uvc_video_decode_data.
               * the solution is an workaround solution, if the length is an error length, we will not decode this data.
		  */
		if((urb->iso_frame_desc[i].actual_length != 0x3FFE) &&
		...
		{
			uvc_trace(UVC_TRACE_LOST, "USB isochronous frame lost %ux%u bytes", 
			urb->iso_frame_desc[i].actual_length, urb->iso_frame_desc[i].length);
		}
