...
// 	struct usb_hcd		*secondary_hcd;
	...
	//power not enable, it's too early to turn on here
	//maybe some HDD power not droped yet. power will be enabled in SUSPEND_POST stage in USB20 driver
...
//	if (!hibernated) {
		...
		regval = readl(0xF0076410);
		regval &= ~(0x2);	 
		writel(regval, 0xF0076410);
#if 0
		...
		struct usb_host_endpoint *ep)
{
    ...
    printk("[USB] Port-2: idVendor=0x%04X, idProduct=0x%04X, bcdDevice=0x%04X.\n", 
    ...
    if(udev->speed == USB_SPEED_SUPER)
        writel(0x09060005, 0xf00707b4);
    else if(udev->speed == USB_SPEED_HIGH)
        writel(0x09060003, 0xf00707b4);
#endif
#ifdef CONFIG_ARCH_MT5399
	if((usb_endpoint_xfer_int(&ep->desc))&&
...
if((ssusb_adb_flag==1) && (MUSB_GADGET_PORT_NUM == 2))
	return 0;
//    SSUSB_VbuscheckGPIO();
